...
class Config:
    ...
    if os.environ.get('DISCORD_CLIENT_ID') is not None:
        _CLIENT_ID = os.environ.get('DISCORD_CLIENT_ID')
    else:
        _CLIENT_ID = 'CLIENT_ID_HERE'

    # the absolute max is 21 due to embedded message limits, 15 is less arbitrary
    ROLL_CAP = 15

    # the character to split dice rolls on, e.g. 2d20 or 2x20
    ROLL_SPLIT = 'd'
